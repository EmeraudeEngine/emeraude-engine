...
- **RenderableInstance**: An instance of a Renderable with unique transformation and parameters. Enables efficient rendering of duplicates. See @docs/renderable-instance-system.md for detailed architecture.
...
**Two implementations:**
- **Unique**: Classic rendering (1 instance, push constants for matrices) - used by `Visual` component
- **Multiple**: GPU instancing (N instances, VBO for matrices) - used by `MultipleVisuals` component
...
- Rendering flags (lighting, shadows, depth test, etc.)
- Shader programs per RenderTarget (generated by Saphir)

**Key Features:**
- Lightweight (only stores transform + flags, shares everything else)
- Many instances → one Renderable (efficient instancing with Multiple)
- Forward multi-pass rendering (1 ambient pass + N light passes)
- Dynamic program generation based on scene configuration (lights, shadows)
- **Cubemap multiview support**: Adapts push constants based on render target type
...
RenderableInstance #1 (Unique):
    Renderable: "wooden_crate"
    Transform: position (10, 0, 5), rotation (0°, 45°, 0°), scale 1.0
    Flags: EnableLighting, EnableShadows

RenderableInstance #2 (Multiple with 1000 instances):
    Renderable: "wooden_crate"
    Transforms: [1000 positions in VBO]
    Flags: EnableLighting, EnableInstancing

Result:
- 1000 crates rendered efficiently via GPU instancing
- Single Geometry, Material, Pipeline (shared)
- 1000 unique transforms stored in VBO
```

> **For detailed RenderableInstance architecture**, see @docs/renderable-instance-system.md
...
- **Shadow maps**: Render depth from light's perspective for shadows
- **Post-processing**: Render scene to texture, apply effects, display result
- **Reflections/Mirrors**: Render scene from mirror's perspective, apply to surface
- **Multi-camera setups**: Security cameras, TV screens, portals showing different views
- **Cubemaps**: 360° environment captures for skyboxes, reflections, and global illumination

### Render Target Types

**1. Texture (2D Render-to-Texture)**
```cpp
// Single 2D texture target (e.g., security camera feed)
auto cameraFeed = RenderTarget::Texture::create(
    "security_camera",
    512, 512,                    // Resolution
    precisions,                  // Color/depth/stencil formats
    viewDistance,                // Max viewable distance
    isOrthographic              // Projection type
);

// Use as texture in material
material->setTexture("diffuse", cameraFeed->colorTexture());
```

**2. ShadowMap (Depth-Only)**
```cpp
// Optimized for shadow mapping (no color attachment)
auto shadowMap = RenderTarget::ShadowMap::create(
    "directional_shadow",
    2048, 2048,                 // High resolution for sharp shadows
    DepthPrecision::D32_SFLOAT  // 32-bit float depth
);

// Used in shadow pass
renderer.renderShadowCasters(shadowMap, lightViewProjection);
```

**3. View (On-Screen/Swapchain)**
```cpp
// Represents the main window/screen
auto mainView = RenderTarget::View::create(
    window,                     // OS window handle
    swapchain,                  // Vulkan swapchain
    vsyncEnabled               // V-sync toggle
);

// Default render target for scene rendering
renderer.setMainRenderTarget(mainView);
```

### Render-to-Texture Architecture

#### Image Layout Management (Critical!)

Render targets must transition between different Vulkan image layouts depending on usage:

**Layout States:**
1. **UNDEFINED** → Initial state after creation (contents invalid)
2. **COLOR_ATTACHMENT_OPTIMAL** → Used during rendering (write depth/color)
3. **SHADER_READ_ONLY_OPTIMAL** → Used as texture in shaders (read-only)

**Transition Pattern:**
```
Creation
    ↓
UNDEFINED → SHADER_READ_ONLY_OPTIMAL (initial transition)
    ↓
┌─────────────────────────────────────┐
│ Rendering Cycle (repeated)          │
│                                      │
...
**Implementation (Texture.cpp):**
```cpp
// Initial transition after image creation
transferManager.transitionImageLayout(
    *m_colorImage,
...
// RenderPass configuration oscillates layout automatically
attachmentDescription.initialLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
attachmentDescription.finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
// During RenderPass: transitions to COLOR_ATTACHMENT_OPTIMAL automatically
```

#### GPU Synchronization

**Problem:** Main scene render might sample render-to-texture before rendering completes.

**Solution:** Semaphore-based GPU synchronization
```cpp
// Each render target has a semaphore signaled when rendering completes
auto semaphore = renderTarget->semaphore();

// Main render waits for all render-to-texture completions
renderer.waitForRenderTargets({cameraFeed, mirrorReflection});
// GPU stalls fragment shader stage until render targets ready
```

**Wait Stages:**
- **Render-to-texture semaphores**: `VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT` (wait when sampling texture)
- **Swapchain semaphores**: `VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT` (wait when writing to framebuffer)

**Code (Renderer.cpp):**
```cpp
// Build wait stages for mixed semaphores
StaticVector<VkPipelineStageFlags, 16> waitStages;

// All render-to-texture semaphores wait at FRAGMENT_SHADER stage
for (auto& rtSemaphore : currentFrameScope.secondarySemaphores()) {
    waitStages.emplace_back(VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
}

// Swapchain semaphore waits at COLOR_ATTACHMENT_OUTPUT stage
currentFrameScope.secondarySemaphores().emplace_back(
    currentFrameScope.imageAvailableSemaphore()->handle()
);
waitStages.emplace_back(VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);

// Submit with proper synchronization
queue->submit(*commandBuffer, SynchInfo{}
    .waits(currentFrameScope.secondarySemaphores(), waitStages)
    .signals(currentFrameScope.renderFinishedSemaphore())
    .withFence(currentFrameScope.inFlightFence())
);
```

#### Known Limitation: Recursive Rendering

**Problem:** Texture showing itself creates infinite recursion
```
SecurityCamera films → TV screen
TV screen displays  → SecurityCamera feed (showing itself!)
Result: Validation warning (texture accessed before render completes)
```

**Current Status:** Works but triggers validation warning. Documented as TODO.

**Future Solution:** Double buffering
```cpp
// Two textures alternating each frame
Frame N: Render to textureA, display textureB
Frame N+1: Render to textureB, display textureA
// Always reading from previous frame (safe)
```

### Cubemap Rendering with Multiview

#### Purpose
Render 360° environment in single pass for:
- **Skyboxes**: Environment backgrounds
- **Reflection probes**: Real-time reflections on shiny surfaces
- **Global illumination**: Light bouncing from environment

#### Vulkan Multiview Extension

**Traditional Approach (6 Render Passes):**
```cpp
// Render each cubemap face separately (SLOW)
for (int face = 0; face < 6; ++face) {
    beginRenderPass(cubemap, face);
    renderScene(faceViewProjection[face]);
    endRenderPass();
}
// 6 render passes × full scene = expensive!
```

**Multiview Approach (1 Render Pass):**
```cpp
// Render all 6 faces simultaneously (FAST)
beginRenderPass(cubemap);  // All 6 layers at once
renderScene();  // Single scene submission
endRenderPass();
// GPU renders to 6 faces in parallel using gl_ViewIndex
```

**How Multiview Works:**
1. **Layered framebuffer**: Cubemap has 6 image layers (+X, -X, +Y, -Y, +Z, -Z)
2. **RenderPass multiview**: Configured with `viewMask = 0x3F` (6 views enabled)
3. **Shader access**: Built-in `gl_ViewIndex` (0-5) identifies current face
4. **Per-face data**: Uniform buffer contains array of 6 view matrices

#### Cubemap Uniform Block Structure

**Problem:** Each face needs different view/projection but shares ambient lighting.

**Solution:** Array of per-face data + shared data
```glsl
// Per-face data structure
struct CubemapFace
{
    mat4 projectionMatrix;        // Different per face
    vec4 positionWorldSpace;      // Camera position (same for all)
    vec4 velocity;                // Camera velocity
    vec4 viewProperties;          // FOV, near, far, etc.
    vec4 _padding;                // Alignment
};

// Uniform block layout
layout(std140, set = 0, binding = 0) uniform CubemapView
{
    CubemapFace faces[6];         // Per-face data (indexed by gl_ViewIndex)
    vec4 ambientLightColor;       // Shared data (same for all faces)
    float ambientLightIntensity;  // Shared data
} ubView;
```

**Shader Access Pattern:**
```glsl
// Access per-face data using gl_ViewIndex
mat4 projection = ubView.faces[gl_ViewIndex].projectionMatrix;
vec4 cameraPos = ubView.faces[gl_ViewIndex].positionWorldSpace;

// Access shared data directly (no indexing)
vec4 ambient = ubView.ambientLightColor * ubView.ambientLightIntensity;
```

**Generator Code (Abstract.cpp):**
```cpp
if (m_renderTarget->isCubemap()) {
    // Create per-face structure
    Declaration::Structure structure{Struct::CubemapFace, "faces"};
    structure.addMember(VariableType::Matrix4, "projectionMatrix");
    structure.addMember(VariableType::FloatVector4, "positionWorldSpace");
    // ... other per-face members

    // Add as array to uniform block
    Declaration::UniformBlock uniformBlock{...};
    uniformBlock.addArrayMember(structure, 6);  // Array of 6 faces

    // Add shared data outside array
...
**Variable Access Helpers (Keys.hpp):**
```cpp
// For 2D/single view rendering
ViewUB("projectionMatrix")
// Generates: ubView.projectionMatrix

// For cubemap multiview rendering
CubeViewUB("faces", "gl_ViewIndex", "projectionMatrix")
// Generates: ubView.faces[gl_ViewIndex].projectionMatrix
```

#### Shader Variable Synthesis Challenge

**Problem:** Some shader code uses push constants for matrices, some uses uniform blocks.

**Solution Implemented:** The RenderableInstance system now uses a context-based approach:

```cpp
// RenderPassContext carries render target information
struct RenderPassContext {
    const CommandBuffer * commandBuffer;
    const ViewMatricesInterface * viewMatrices;
    uint32_t readStateIndex;
    bool isCubemap;  // Key flag for cubemap detection
};

// Push constant behavior adapts based on isCubemap flag
if (passContext.isCubemap) {
    // Cubemap: Model only (VP in UBO[gl_ViewIndex])
    // Multiple: Nothing pushed (Model in VBO, VP in UBO)
} else {
    // Classic 2D: MVP or V+M via push constants
}
```

**Remaining work:** Adapt Saphir shader generators to produce shaders using `gl_ViewIndex` for VP matrix access.

### Abstraction Level

**High-level interface** (Graphics layer):
```cpp
// 2D texture for camera feed
auto cameraFeed = RenderTarget::Texture::create("camera", 512, 512, ...);

// Depth-only shadow map
auto shadowMap = RenderTarget::ShadowMap::create("shadow", 2048, 2048, ...);

// Cubemap for environment reflections
auto envProbe = RenderTarget::Cubemap::create("env_probe", 256, ...);

// Use in render passes
renderer.renderToTarget(cameraFeed, cameraView);
renderer.renderShadows(shadowMap, lightView);
renderer.renderCubemap(envProbe, probePosition);

// Textures automatically available for sampling
material->setTexture("reflection", envProbe->cubemapTexture());
```

**Low-level implementation** (Vulkan layer):
- Vulkan framebuffers, render passes, image views managed internally
- Automatic layout transitions (UNDEFINED → SHADER_READ_ONLY → COLOR_ATTACHMENT → SHADER_READ_ONLY)
- Synchronization handled (semaphores ensure render completion before sampling)
- Multiview configuration (viewMask, correlation masks for cubemaps)

### Integration with Scene

RenderTargets are integrated with the engine through:

**1. AVConsole (Audio-Video Virtual Device System)**
- Treats render targets as "virtual video devices"
- Security cameras, TV screens, mirrors all use RenderTarget::Texture
- Scene nodes can "connect" to render targets to display their output

**2. ShadowManager (Part of Saphir)**
- Creates RenderTarget::ShadowMap for each shadow-casting light
- Renders depth-only pass for shadow calculation
- Shadow maps sampled in lighting shaders

**3. Environment System**
- Uses RenderTarget::Cubemap for reflection probes
- Updates cubemaps periodically or on-demand
...
The engine provides two image resource types to serve different texture needs:

**ImageResource** (`Graphics/ImageResource.hpp`):
- Wraps `Pixmap<uint8_t>` for 2D image data
- Used by `Texture1D`, `Texture2D`, and `TextureCubemap`
- Supports loading from image files (PNG, JPEG, TGA)
- Default resource: 64×64 magenta with X pattern (debug) or Perlin noise (release)

**VolumetricImageResource** (`Graphics/VolumetricImageResource.hpp`):
- Stores 3D volumetric data as `std::vector<uint8_t>`
- Explicit dimensions: `width`, `height`, `depth`, `colorCount`
- Used exclusively by `Texture3D`
- Default resource: 32×32×32 RGB gradient cube
- File loading: TODO (raw, vox, DICOM formats planned)

### Texture Types

| Type | Vulkan Image Type | Image Resource | Use Case |
|------|------------------|----------------|----------|
| **Texture1D** | `VK_IMAGE_TYPE_1D` | `ImageResource` | Lookup tables, gradients |
| **Texture2D** | `VK_IMAGE_TYPE_2D` | `ImageResource` | Standard textures, diffuse/normal maps |
| **Texture3D** | `VK_IMAGE_TYPE_3D` | `VolumetricImageResource` | Volume rendering, 3D noise, LUTs |
| **TextureCubemap** | `VK_IMAGE_VIEW_TYPE_CUBE` | `CubemapImageResource` | Skyboxes, environment maps |

### Texture Creation Pattern

All texture types follow the same pattern from `TextureResource::Abstract`:

```cpp
// 1. Load image data (via dependency system)
bool Texture2D::load(ServiceProvider & serviceProvider) {
    m_localData = serviceProvider.container<ImageResource>()->getDefaultResource();
    this->addDependency(m_localData);
    return this->setLoadSuccess(true);
}

// 2. Create Vulkan resources when dependencies loaded
bool Texture2D::createTexture(Renderer & renderer) {
    // Validate texture data
    if (!this->validateTexture(m_localData->data(), ...)) return false;

    // Create Vulkan image
    m_image = std::make_shared<Vulkan::Image>(...);
    m_image->create(renderer.transferManager(), m_localData);

    // Create image view
    m_imageView = std::make_shared<ImageView>(m_image, ...);
    m_imageView->createOnHardware();

    // Get shared sampler
    m_sampler = renderer.getSampler("Texture2D", ...);
...
`Texture3D` differs from 2D textures:
- Uses `VolumetricImageResource` instead of `ImageResource`
- Creates image with `createOnHardware()` then `writeData()` for raw byte transfer
- No anisotropic filtering (not typically used for 3D textures)
- MIP levels calculated from width/height (depth not considered)

```cpp
// Texture3D creation pattern
m_image->createOnHardware();  // Allocate GPU memory
m_image->writeData(transferManager, MemoryRegion{data.data(), data.size()});  // Upload raw bytes
```
...
## Dynamic Viewport and Scissor (Window Resize Optimization)

### Problem: Pipeline Recreation on Resize

Traditional Vulkan implementations require graphics pipeline recreation when the window/swapchain is resized, because viewport and scissor dimensions are baked into the pipeline at creation time. This causes:
- **Performance stutter** during resize operations
- **Complex refresh mechanisms** to track and recreate all pipelines
- **Synchronization challenges** between old and new pipelines

### Solution: Vulkan Dynamic States
...
graphicsPipeline.configureDynamicStates(dynamicStates);

// Runtime (each frame, in command buffer)
VkViewport viewport{};
viewport.x = 0.0F;
viewport.y = 0.0F;
...
**Pipelines with dynamic states:**
- `SceneRendering` - Main 3D scene rendering (`src/Saphir/Generator/SceneRendering.cpp`)
- `GizmoRendering` - Debug gizmo rendering (`src/Saphir/Generator/GizmoRendering.cpp`)
- `TBNSpaceRendering` - Tangent/Binormal/Normal debug visualization (`src/Saphir/Generator/TBNSpaceRendering.cpp`)
- `ShadowCasting` - Shadow map generation (`src/Saphir/Generator/ShadowCasting.cpp`)

**Viewport/scissor configuration:**
- `RenderTarget::Abstract::setViewport()` - Sets both viewport AND scissor dynamically (`src/Graphics/RenderTarget/Abstract.cpp:86-110`)
- Called from `RenderableInstance::Abstract` at each pipeline bind point

**Key code references:**
- `src/Graphics/RenderTarget/Abstract.cpp:setViewport()` - Dynamic viewport/scissor configuration
- `src/Graphics/RenderableInstance/Abstract.cpp` - Calls `setViewport()` at pipeline bind points
- `src/Core.cpp:onWindowChanged()` - No longer calls `refreshActiveScene()` (pipelines don't need recreation)

### Benefits

| Aspect | Before (Static) | After (Dynamic) |
|--------|-----------------|-----------------|
| **Resize performance** | Stutters (pipeline recreation) | Smooth (no recreation) |
| **Code complexity** | RefreshGraphicsPipelines mechanism needed | Simple, no refresh logic |
| **Memory** | Multiple pipeline versions during transition | Single pipeline, reused |
| **Debug builds** | Very slow resize | Fast resize |

### Why This Works

Vulkan pipelines are compatible with any RenderPass that has the same **attachment configuration** (format, samples, load/store ops). The viewport and scissor dimensions are NOT part of RenderPass compatibility, so:

1. Pipeline created with dynamic viewport/scissor → viewport dimensions not baked in
2. Window resizes → swapchain recreated with new dimensions
3. RenderPass remains compatible (same attachments, just different size)
4. At render time → `vkCmdSetViewport/Scissor` configures current dimensions
5. No pipeline recreation needed

### Related: 2D Overlay System

The 2D overlay system (ImGui-based) already used dynamic states and was unaffected by this optimization. The 3D scene rendering pipelines were the ones that benefited from this change.
...
| **Dynamic States** | Viewport/scissor set at runtime, not pipeline creation | No pipeline recreation on resize |
