...
- **LoadingRequest**: Internal wrapper for asynchronous loading operations. Handles file loading, network downloads (⚠️ the `ExternalData` download chain is **not functional today** — `Net::Manager` never emits `FileDownloaded`; see `src/Net/AGENTS.md` § Download manager and `docs/todo/net-manager-download-chain-broken.md`), and caching. Lives in `LoadingRequest.hpp/.cpp` and is **deliberately not a template** — it only needs the polymorphic `ResourceTrait` plus the resource's `ClassId` — so that its heavy dependencies (`FileSystem`, `Network/URL`, `String`, `IO`) stay out of `Container.hpp`.
