...
- **LoadingRequest**: Internal wrapper for asynchronous loading operations. Handles file loading, network downloads (`"Source": "ExternalData"` — the file is fetched by `Net::Manager` over HTTPS into its URL-keyed cache, and the request is rewritten to `LocalData` on the cached file when `FileDownloaded` arrives; see `src/Net/AGENTS.md` § Download manager) and direct data. Lives in `LoadingRequest.hpp/.cpp` and is **deliberately not a template** — it only needs the polymorphic `ResourceTrait` plus the resource's `ClassId` — so that its heavy dependencies (`FileSystem`, `Network/URL`, `String`, `IO`) stay out of `Container.hpp`.
