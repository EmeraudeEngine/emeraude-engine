...
> below. Inter-cascade blending (§ *B3*) and the per-cascade depth bias (§ *the knob that did
> nothing*) have since landed too. **The polish pass now owes exactly ONE thing: a single source of
> truth for the light-space transform** — re-verified against the code 2026-09-08, it is the only
> item of the CSM rework still open. ⚠️ It is computed at **FOUR** sites, not three as this line
...
| 4a | `updateCascades()` fills `m_CSMBuffer` every frame but **never calls `requestVideoMemoryUpdate()`**, and `AbstractLightEmitter::updateVideoMemory()` uploads nothing without that flag. The early return is NOT the cause: when `shadowMapResolution > 0` the map exists and the function runs to completion | The only upload ever performed is the one from `createOnHardware()`, at a point where `updateCascades()` has not run yet and `m_CSMBuffer` is **all zeros** — black colour, zero intensity, null matrices. Hence "no light at all", permanently, for a STATIC light | **FIXED** `c7eef938` (pushed) — `updateCascades()` now ends on `requestVideoMemoryUpdate()`. The falsifiable prediction that isolated this cause from every other: a CSM light in **continuous motion** lit correctly even while a static one stayed black, because `move()` raises the flag every frame |
| 4b | `ViewMatricesCascadedUBO::computeCascadeProjection()` passes `minZ - margin` / `maxZ + margin` as the ortho near/far, but those are **view-space z coordinates** (negative in front of the camera) while `orthographicProjection()` expects positive **distances** | Cascade depth lands outside [0,1]: casters are clipped out of the map at render time, and at sampling time the `projCoords.z >= 0 && <= 1` guard fails, leaving `shadowFactor = 1.0`. No shadow, ever — independently of 4a | **FIXED** `8c6417f0` (pushed). The bounds were first corrected to `near = -maxZ - margin`, `far = -minZ + margin` (sign **and** order inverted); the rotation-invariant fit then superseded that by **anchoring the light camera on the snapped centre**, which makes the near plane trivially the camera and the range exactly `3 × radius`. `frustumCenter` is no longer dead — it is what the camera is anchored on |
...
| A1 | **FIXED.** Not CSM-only: EVERY light's sampling matrix was outside the contract. The CSM matrix existed **twice, on two different regimes**: the rasterising copy is published (`m_renderState[readStateIndex]`), the sampling copy (`DirectionalLight::m_CSMBuffer`) is not. `cascadeViewProjectionMatrix()` has **no `readStateIndex` overload** and serves `m_logicState`, unlike every other accessor in that class. The claim that **no light emitter had ever overridden `publishStateForRendering()`** is what changed: `AbstractLightEmitter` now overrides it (`AbstractLightEmitter.cpp:102`), writing the block into one published slot per frame-in-flight, gated on a logic generation. `cascadeViewProjectionMatrix()` still has no `readStateIndex` overload and still serves `m_logicState` — which is **correct and not a residual race**: its only caller, `DirectionalLight::updateCascades()`, runs on the LOGIC thread and publishes afterwards. Re-verified 2026-09-08. | **FIXED** |
