...
**Purpose:** Set up Release configuration for optimized compilation.
...
# Linux (with tests):
cmake -S . -B .claude-build-release -DCMAKE_BUILD_TYPE=Release -DEMERAUDE_ENABLE_TESTS:BOOL=ON -G "Ninja"
...
cmake -S . -B .claude-build-release -DCMAKE_BUILD_TYPE=Release -DCMAKE_OSX_ARCHITECTURES=arm64 -DEMERAUDE_ENABLE_TESTS:BOOL=ON -G "Ninja"
...
cmake -S . -B .claude-build-release -DCMAKE_BUILD_TYPE=Release -DCMAKE_OSX_ARCHITECTURES=x86_64 -DEMERAUDE_ENABLE_TESTS:BOOL=ON -G "Ninja"

# Windows (without tests):
cmake -S . -B .claude-build-release -DCMAKE_BUILD_TYPE=Release -G "Visual Studio 17 2022" -A x64

# Windows (with tests):
cmake -S . -B .claude-build-release -DCMAKE_BUILD_TYPE=Release -DEMERAUDE_ENABLE_TESTS:BOOL=ON -G "Visual Studio 17 2022" -A x64
```
...
**Next step:** Run `/build-release` to compile.