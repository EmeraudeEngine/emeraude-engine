...
namespace EmEn::Vulkan
{
	...
	class DeviceQueueConfiguration final
	{
		public:
			...
			{
				m_queueByPriorities[static_cast< uint32_t >(priority)].emplace_back(queue);
			}
			...
			{
				return m_queueByPriorities[static_cast< uint32_t >(priority)];
			}

			/**
			 * @brief Returns a queue by priority.
			 * @param priority The priority desired. High, Medium or Low.
			 * @param queueIndex The queue index. Default 0.
			 * @return Queue *
			 */
			[[nodiscard]]
			Queue * queue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept;
			...
			clear () noexcept
			{
				...
				for ( auto & queueList : m_queueByPriorities )
				{
					queueList.clear();
				}
			}
		...
		private:
			...
			std::array< Libs::StaticVector< Queue *, 16 >, 3 > m_queueByPriorities;
	};
	...
	class Device final : public std::enable_shared_from_this< Device >, public AbstractObject, public Libs::NameableTrait
	{
		public:
			...
			{
				/*return std::ranges::all_of(m_queueStructures, [] (const auto & queueConfiguration) {
					return queueConfiguration.queueFamilyIndex() == 0;
				});*/
			...
			/**
			 * @brief Returns whether the device has been set up for graphics and presentation.
			 * @return bool
			 */
			[[nodiscard]]
			bool
			hasGraphicsAndPresentationQueues () const noexcept
			{
				/* TODO: For now, this is the same as the graphics queue. */
				return m_graphicsQueueConfiguration.enabled();
			}

			/**
			 * @brief Returns the queue family index for graphics and presentation.
			 * @note This may return the same family queue index from another configuration.
			 * @warning Be sure of calling Device::hasGraphicsAndPresentationQueues() before trusting the index.
			 * @return uint32_t
			 */
			[[nodiscard]]
			uint32_t
			getGraphicsAndPresentationFamilyIndex () const noexcept
			{
				/* TODO: For now, this is the same as the graphics queue. */
			 ...
			 * @param priority The priority of the queue.
			 * @param queueIndex The index of the queue. Default 0.
			 * @return Queue *
			 */
			[[nodiscard]]
			Queue *
			getGraphicsAndPresentationQueue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept
			{
				/* TODO: For now, this is the same as the graphics queue. */
				...
				return m_graphicsQueueConfiguration.queue(priority, queueIndex);
			}
			...
			[[nodiscard]]
			Queue *
			getGraphicsQueue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept
			{
				if ( !m_graphicsQueueConfiguration.enabled() )
				{
					return nullptr;
			...
			[[nodiscard]]
			Queue *
			getComputeQueue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept
			{
				...
				return m_computeQueueConfiguration.queue(priority, queueIndex);
			}
			...
			[[nodiscard]]
			Queue *
			getTransferQueue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept
			{
				...
				return m_transferQueueConfiguration.queue(priority, queueIndex);
			}
			...
			[[nodiscard]]
			Queue *
			getGraphicsTransferQueue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept
			{
				if ( !m_transferQueueConfiguration.enabled() )
				{
					return m_graphicsQueueConfiguration.queue(priority, queueIndex);
			...
			[[nodiscard]]
			Queue *
			getComputeTransferQueue (QueuePriority priority, uint32_t queueIndex = 0) const noexcept
			{
				if ( !m_transferQueueConfiguration.enabled() )
				{
					return m_computeQueueConfiguration.queue(priority, queueIndex);
