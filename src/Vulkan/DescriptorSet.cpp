...
namespace EmEn::Vulkan
{
	...
	DescriptorSet::create () noexcept
	{
		...
		this->setVulkanObjectName(m_descriptorPool->device()->handle(), VK_OBJECT_TYPE_DESCRIPTOR_SET, reinterpret_cast< uint64_t >(m_handle));
	...
	bool
	DescriptorSet::writeAccelerationStructure (uint32_t bindingIndex, VkAccelerationStructureKHR accelerationStructure) const noexcept
	{
		if ( !this->isCreated() )
		{
			Tracer::error(ClassId, "The descriptor set is not yet created ! Unable to write into it.");
		...
		{
			Tracer::error(ClassId, "The acceleration structure handle is null !");
