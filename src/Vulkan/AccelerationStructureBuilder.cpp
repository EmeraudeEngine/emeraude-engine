...
namespace EmEn::Vulkan
{
	...
	AccelerationStructureBuilder::buildTLAS (const std::vector< TLASInstanceInput > & instances) noexcept
	{
		...
		if ( instances.empty() )
		{
			Tracer::error(ClassId, "No instances provided for TLAS build !");
		...
		/* 2. Create and upload instance buffer (local, destroyed after synchronous fence wait). */
		...
		auto tlas = std::make_unique< AccelerationStructure >(m_device, VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR, buildSizesInfo.accelerationStructureSize);

		if ( !tlas->createOnHardware() )
		{
		...
		/* 6. Create scratch buffer (local, destroyed after synchronous fence wait). */
		constexpr VkDeviceSize ScratchAlignment = 256;
		const auto scratchAllocSize = buildSizesInfo.buildScratchSize + ScratchAlignment;
		...
		{
			Tracer::error(ClassId, "TLAS build command submission failed !");
