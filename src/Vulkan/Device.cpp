...
namespace EmEn::Vulkan
{
	...
	Queue *
	DeviceQueueConfiguration::queue (QueuePriority priority, uint32_t queueIndex) const noexcept
	{
		...
		for ( const uint8_t index : searchOrder )
		{
			if ( const auto & queueList = m_queueByPriorities[index]; !queueList.empty() )
			{
				if ( queueIndex >= queueList.size() )
				{
					return queueList.back();
				}

				return queueList[queueIndex];
			}
		}
	...
	Device::installQueues (const std::map< uint32_t, StaticVector< float, 16 > > & queuePriorityValues, DeviceQueueConfiguration & configuration) noexcept
	{
		...
		for ( uint32_t queueIndex = 0; queueIndex < queueCount; queueIndex++ )
		{
			...
			const auto & queue = m_queues.emplace_back(std::make_unique< Queue >(queueHandle, queueFamilyIndex));
