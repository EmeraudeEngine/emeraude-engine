...
namespace EmEn::Scenes::Component
{
	...
	class DirectionalLight final : public AbstractLightEmitter
	{
		public:
			 ...
			 * @param cascadeCount The number of cascades (1-4) when using CSM. Default is 4.
			 * @param lambda The split factor (0 = linear, 1 = logarithmic, 0.5 = balanced) when using CSM. Default is 0.5.
			 */
			DirectionalLight (const std::string & componentName, const AbstractEntity & parentEntity, uint32_t shadowMapResolution, float coverageSize = 0.0F, uint32_t cascadeCount = Graphics::MaxCascadeCount, float lambda = Graphics::DefaultCascadeLambda) noexcept
				...
				m_usesCSM{coverageSize <= 0.0F && shadowMapResolution > 0}
			{
