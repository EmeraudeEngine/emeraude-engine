...
namespace EmEn::Scenes::Component
{
	...
	bool
	PointLight::touch (const Space3D::Sphere< float > & target, uint32_t readStateIndex) const noexcept
	{
	...
	PointLight::createOnHardware (Scene & scene) noexcept
	{
		...
		{
			m_shadowMap = scene.createRenderToCubicShadowMap(this->name() + ShadowMapName, resolution, this->getDistanceOrFar());
