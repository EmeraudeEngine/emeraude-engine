...
- `updateVisibility(cameraPosition)` - LOD/visibility hint (logic thread, once per cycle; `TerrainResource` slides its streamed window from here through the engine `ThreadPool`, never a raw `std::thread` — the publication is the render thread's, `docs/caution-points.md` § Ray Tracing)
...
- **Subclasses**: `VertexGridResource` overrides `generateTriangleListIndicesForRT()` for strip conversion; `AdaptiveVertexGridResource` (every terrain floor) overrides it too — **since 2026-09-22, before which it inherited the base returning `{}` and every terrain was silently absent from the TLAS** while the log repeated `uses TriangleStrip but generateTriangleListIndicesForRT() returned empty indices` (2237 times in a 2-minute `forest` run: `SceneMetaData` retries the build on every TLAS refresh).
  - ⚠️⚠️ **Its IBO cannot be converted wholesale**, unlike `VertexGridResource`'s: it holds, per sector, ONE strip range PER LOD LEVEL over the SAME quads, plus the edge-stitching ranges. Converting all of it would stack every LOD of the terrain inside one BLAS. The override therefore **regenerates a single proxy from the grid**, ignoring the IBO entirely, at the finest step whose triangle count fits `Core/Graphics/RayTracing/TerrainBLASMaxTriangles` (default 2 000 000). A BLAS has no view-dependent LOD selection, so one fixed step is the only coherent content.
  - The VBO holds one vertex per grid point in grid-point order, so a `Grid::index(x, y)` **is** a VBO index; the winding follows the raster strip — `(T0, B0, T1)` then `(B0, B1, T1)`.
  - ⚠️ **Cost measured on `terrain` (4096 divisions, 2026-09-22)**: step 1 = 33 554 432 triangles and **7.5 GiB** of VRAM against **5.05 GiB** at the default budget (step 8, 524 288 triangles) — idle 1.9 GiB, same machine, an 8 GiB card. The full-resolution surface is quadratic in the division count and unbounded. `forest` (128 divisions, 32 768 triangles) keeps step 1 either way.
  - **A geometry that REPLACES its vertex data rebuilds its BLAS through the frame path** (2026-09-22). The worker that built the new window only STAGES it (`updateData()`); the RENDER thread publishes it in `updateVideoMemory()` — reached through `Renderer::requestGeometryVideoMemoryUpdate()` / `flushGeometryVideoMemoryUpdates()`, the geometries' twin of the materials' flush — and calls `Interface::markAccelerationStructureStale()` as its LAST statement (the flag is what publishes the new grid and the new VBO), so `SceneMetaData::rebuild()` consumes it next to the on-demand build of a missing BLAS: `if ( blas == nullptr || geometry->isAccelerationStructureStale() )`. `buildAccelerationStructure()` clears the flag as soon as it commits, and retires the old BLAS **and** the old RT index buffer through the `DeferredDestructor` — a frame in flight may still be reading both.
    - A REBUILD, not a refit: a slide is rare (`visibleSize / 3` of travel) while a refit freezes a BVH partition built for another window. Refit is the tool of per-frame motion — skinning — and stays there (owner decision, 2026-09-22).
    - Measured on `forest` with the visible window shrunk to force slides: with the flag, every `Threshold reached` is followed by one proxy rebuild (2 slides → 2 rebuilds); without it, 4 slides → 0. 0 VUID either way. ⚠️ The IMAGES of two such runs are NOT comparable — the runs do not slide the same number of times, so they show different windows of the terrain; the proof is at log level.
