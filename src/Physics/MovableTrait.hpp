...
namespace EmEn::Physics
{
	...
	class MovableTrait
	{
		public:
			...
			/**
			 * @brief [NEW] Applies a linear impulse directly to the velocity.
			...
			/**
			 * @brief [NEW] Applies an angular impulse directly to the angular velocity.
			...
			/**
			 * @brief [NEW] Updates the inverse world inertia tensor from the current orientation.
			...
			/**
			 * @brief [NEW] Returns the inverse world inertia tensor.
			...
			/**
			 * @brief Sets the world position after physics simulation (public accessor for physics engine).
			 * @param worldPosition A reference to a vector.
			 * @return void
			 */
			void
			applySimulatedPosition (const Libs::Math::Vector< 3, float > & worldPosition) noexcept
			{
				this->simulatedMove(worldPosition);
			}
