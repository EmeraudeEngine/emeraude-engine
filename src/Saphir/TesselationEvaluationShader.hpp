/*
 * src/Saphir/TesselationEvaluationShader.hpp
...
namespace EmEn::Saphir
{
	...
	class TesselationEvaluationShader final : public AbstractShader
	{
		public:
			...
			static constexpr auto ClassId{"TesselationEvaluationShader"};

			/** @brief Extends declaration from base class. */
			using AbstractShader::declare;

			/**
			 * @brief Constructs a tesselation evaluation shader.
			 ...
			 */
			TesselationEvaluationShader (std::string name, std::string GLSLVersion, std::string GLSLProfile) noexcept
			...
			/**
			 * @brief Copies output from a tesselation control shader to this fragment shader.
			 * @param tesselationControlShader A reference to a tesselation control shader.
			 * @return bool
			 */
			[[nodiscard]]
			bool connectFromPreviousShader (const TesselationControlShader & tesselationControlShader) noexcept;
