...
namespace EmEn::Saphir
{
	...
	bool
	LightGenerator::generatePhongBlinnVertexShader (Generator::Abstract & generator, VertexShader & vertexShader, LightType lightType, bool enableShadowMap, bool enableColorProjection) const noexcept
	{
	...
	bool
	LightGenerator::generatePhongBlinnFragmentShader (Generator::Abstract & generator, FragmentShader & fragmentShader, LightType lightType, [[maybe_unused]] bool enableShadowMap, [[maybe_unused]] bool enableColorProjection) const noexcept
	{
		...
		if ( !m_surfaceSpecularColor.empty() )
		{
			...
			Code{fragmentShader} <<
				...
				"	" << SpecularFactor << " = pow(max(dot(twoSidedN, H), 0.0), specularExponent) * ((specularExponent + 2.0) / 25.132741228718345) * " << DiffuseFactor << ';' << Line::End <<
				'}' << Line::End;
		}
