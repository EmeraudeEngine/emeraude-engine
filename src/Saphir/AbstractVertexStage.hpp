/*
 * src/Saphir/AbstractVertexStage.hpp
...
namespace EmEn::Saphir
{
	...
	/**
	 * @brief The PER-VERTEX stage feeding the rasterizer: what the vertex shader and the mesh shader share.
	 * @note It holds the whole synthesis machinery — requestSynthesizeInstruction() and the ~20 synthetic
	 * variables (positions in every space, tangent frames, texture coordinates, velocity clip positions),
	 * the model-matrix sources and the displacement chain (skinning, wind, heightfield). The material, light
	 * and shadow generators address THIS type, never a concrete stage, so they serve both pipelines
	 * (owner decision, 2026-09-22). A concrete stage decides only how its per-vertex values reach the next
	 * stage.
	 ...
	 * @version 0.9.54
	 */
	class AbstractVertexStage : public AbstractShader
	{
		public:

			/** @brief Class identifier. */
			static constexpr auto ClassId{"VertexStage"};
		...
		protected:
			...
			/**
			 * @brief Constructs a per-vertex stage.
			 ...
			 */
			AbstractVertexStage (std::string name, std::string GLSLVersion, std::string GLSLProfile) noexcept
