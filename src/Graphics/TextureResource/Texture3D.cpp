...
namespace EmEn::Graphics::TextureResource
{
	...
	Texture3D::createTexture (Renderer & renderer) noexcept
	{
		...
		m_image = std::make_shared< Vulkan::Image >(
			renderer.device(),
			VK_IMAGE_TYPE_3D,
			Image::getFormat< uint8_t >(m_localData->colorCount(), this->isSRGB()),
	...
	bool
	Texture3D::destroyTexture () noexcept
	{
		if ( m_image != nullptr )
		{
			m_image->destroyFromHardware();
	...
	Texture3D::load () noexcept
	{
		...
		m_localData = this->serviceProvider().container< VolumetricImageResource >()->getDefaultResource();
	...
	{
		return this->load(this->serviceProvider().container< VolumetricImageResource >()->getResource(
