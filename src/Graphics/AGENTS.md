...
### Light Attenuation — ⚠️⚠️ NOT physical since 2026-08-12 (the section below describes what was LOST)

> ⚠️⚠️ **FALSE since `1c1d94ba` (2026-08-12, "the Blinn-Phong machinery is gone")**: that commit deleted
> `LightGenerator.PerFragment.cpp`, where the windowed inverse square below lived. The ONLY falloff generated today
> is `LightFactor *= max(1.0 - dot(DR, DR), 0.0)` with `DR = |d| / radius` (`LightGenerator.PBR.cpp` ~446-454;
> RTGI/RTR/probes do the same): radius-bounded, NO `1/d²`. A point or spot intensity in candela is therefore not a
> physical quantity — a light delivers about its full intensity at any distance well inside its radius, and the
> radius acts as a dimmer. Found 2026-09-25 by the light-colour normalisation audit; restoring the inverse square
> changes every point/spot-lit scene (owner decision): item `point-spot-falloff-lost-inverse-square`.

Point and spot lights fell off as a **windowed inverse square** (Karis, "Real Shading in Unreal
