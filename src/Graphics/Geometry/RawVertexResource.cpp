/*
 * src/Graphics/Geometry/RawVertexResource.cpp
...
namespace EmEn::Graphics::Geometry
{
	...
	bool
	RawVertexResource::createOnHardware (TransferManager & /*transferManager*/) noexcept
	{
		if ( this->isCreated() )
		{
			return true;
	...
	bool
	RawVertexResource::updateVideoMemory () noexcept
	{
		if ( !this->isCreated() )
		{
	...
	void
	RawVertexResource::destroyFromHardware (bool clearLocalData) noexcept
	{
	...
	bool
	RawVertexResource::load () noexcept
	{
	...
	bool
	RawVertexResource::load (const std::filesystem::path & /*filepath*/) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
	...
	bool
	RawVertexResource::load (const Json::Value & /*data*/) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
	...
	bool
	RawVertexResource::load (std::span< const float > vertexAttributes, uint32_t vertexElementCount, const RawGeometryOptions & options) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
		...
		return this->setLoadSuccess(this->uploadToGPU(vertexAttributes.data(), vertexCount, vertexElementCount, options));
	}
	...
	bool
	RawVertexResource::load (std::span< const float > positions, std::span< const float > normals, std::span< const float > textureCoordinates, std::span< const float > colors, const RawGeometryOptions & options) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
		...
		return this->setLoadSuccess(this->uploadToGPU(interleaved.data(), vertexCount, vertexElementCount, options));
	}
	...
	bool
	RawVertexResource::uploadToGPU (const float * vertexAttributes, uint32_t vertexCount, uint32_t vertexElementCount, const RawGeometryOptions & options) noexcept
	{
	...
	void
	RawVertexResource::computeBoundingVolumes (AACuboid< float > & boundingBox, Sphere< float > & boundingSphere, const Point< float > & minimum, const Point< float > & maximum) noexcept
	{
	...
	void
	RawVertexResource::computeBoundingVolumes (AACuboid< float > & boundingBox, Sphere< float > & boundingSphere, const float * positions, uint32_t vertexCount, uint32_t stride) noexcept
	{
		boundingBox.reset();
