 ...
 * (`illuminanceFromIntensity()` below, i.e. inverse square). The shaders apply it — raster and traced lanes alike —
 * through ONE helper, `Graphics/Effects/Shared/LightFalloffGLSL.hpp` (`saturate(1 - (d/r)^4)^2 / max(d², 0.01²)`).
 * ⚠️ It was lost from 2026-08-12 to 2026-09-25 (`max(1 - (d/r)², 0)` only), which made a lumen value arbitrary.
 */
 ...
 * @return float The illuminance, in lux.
	 ...
	 * @warning The result is a culling bound, NOT a dimmer: the shader keeps applying the inverse-square law
	 * inside it, the window only bringing it to zero at the radius (`LightFalloffGLSL.hpp`). Feeding it back as
	 * an attenuation range would darken the scene twice.
	 * @param candela The luminous intensity, in candela.
