/*
 * src/Graphics/Effects/Framebuffer/FXAA.cpp
...
static constexpr auto TracerTag{"FXAAEffect"};
...
namespace
{
	...
	static_assert(sizeof(FXAAPushConstants) == 32, "FXAAPushConstants must be 32 bytes.");

	/* ---- GLSL Shader Sources ---- */

	static constexpr auto FXAAFragmentShader = R"GLSL(
#version 450
...
void main()
{
	...
	/* Check if the center pixel luminance is on the correct side of the edge. */
...
namespace EmEn::Graphics::Effects::Framebuffer
{
	...
	bool
	FXAA::create (uint32_t width, uint32_t height) noexcept
	{
		auto & renderer = this->renderer();

		/* Output is LDR (8-bit per channel). */
		if ( !m_outputTarget.create(renderer, width, height, VK_FORMAT_R8G8B8A8_UNORM, "FXAAOutput") )
		{
			TraceError{TracerTag} << "Failed to create FXAA output target !";
		...
		const auto fragmentModule = renderer.shaderManager().getShaderModuleFromSourceCode(renderer.device(), "FXAAFS", Saphir::ShaderType::FragmentShader, FXAAFragmentShader);

		if ( vertexModule == nullptr || fragmentModule == nullptr )
		{
			TraceError{TracerTag} << "Failed to compile FXAA shaders !";
		...
		/* Graphics pipeline. */
		m_pipeline = this->createFullscreenPipeline(ClassId, "FXAA", vertexModule, fragmentModule, m_pipelineLayout, m_outputTarget);

		if ( m_pipeline == nullptr )
		{
			TraceError{TracerTag} << "Failed to create FXAA pipeline !";

			return false;
		}

		/* Per-frame descriptor sets. */
		m_descriptorSets = this->createPerFrameDescriptorSets(descriptorSetLayout, ClassId, "FXAADescSet");
	...
	FXAA::execute (const CommandBuffer & commandBuffer, const TextureInterface & inputColor, [[maybe_unused]] const TextureInterface * inputDepth, [[maybe_unused]] const TextureInterface * inputNormals, [[maybe_unused]] const TextureInterface * inputMaterialProperties, [[maybe_unused]] const Scenes::LightSet * lightSet, [[maybe_unused]] const PostProcessor::PushConstants & constants) noexcept
	{
		...
		/* Build push constants with computed texel size. */
		const FXAAPushConstants pc{
