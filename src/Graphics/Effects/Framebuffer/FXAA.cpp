...
namespace EmEn::Graphics::Effects::Framebuffer
{
	...
	bool
	FXAA::create (Renderer & renderer, uint32_t width, uint32_t height) noexcept
	{
		...
		auto fragmentModule = renderer.shaderManager().getShaderModuleFromSourceCode(
			renderer.device(), "FXAAFS", Saphir::ShaderType::FragmentShader, FXAAFragmentShader
		);
		...
		m_pipeline = IndirectPostProcessEffect::createFullscreenPipeline(
			renderer, ClassId, "FXAA", vertexModule, fragmentModule, m_pipelineLayout, m_outputTarget
		);
		...
		m_descriptorSets = createPerFrameDescriptorSets(renderer, descriptorSetLayout, ClassId, "FXAADescSet");
		...
		TraceSuccess{TracerTag} << "FXAA effect created (" << width << "x" << height << ").";
