/*
 * src/Scenes/Effect/FogEnvironment.cpp
...
namespace EmEn::Graphics::Effects
{
	...
	bool
	FogEnvironment::generateVertexShaderCode (Generator::Abstract & generator, VertexShader & vertexShader) const noexcept
	{
		/* Fog per vertex */
		if ( !m_highQuality )
		{
	...
	bool
	FogEnvironment::generateFragmentShaderCode (Generator::Abstract & /*generator*/, FragmentShader & fragmentShader) const noexcept
	{
	...
	FogEnvironment::FogEnvironment (FogMode mode, bool highQuality) noexcept
	...
	void
	FogEnvironment::setDensity (float density) noexcept
	{
		if ( density < 0.0F )
		{
	...
	void
	FogEnvironment::setLinearRange (float start, float end) noexcept
	{
		if ( m_mode != FogMode::Linear )
		{
	...
	void
	FogEnvironment::setColor (const PixelFactory::Color< float > & color) noexcept
	{
		m_color = color;
	...
	void
	FogEnvironment::useBackgroundColor () noexcept
	{
		m_useBackgroundColor = true;
	}

	FogEnvironment::FogMode
	FogEnvironment::mode () const noexcept
	{
		return m_mode;
	}

	float
	FogEnvironment::density () const noexcept
	{
		return m_density;
	}

	float
	FogEnvironment::linearStart () const noexcept
	{
		return m_linearStart;
	}

	float
	FogEnvironment::linearEnd () const noexcept
	{
		return m_linearEnd;
	}

	const PixelFactory::Color< float > &
	FogEnvironment::color () const noexcept
	{
		return m_color;
	}
}
