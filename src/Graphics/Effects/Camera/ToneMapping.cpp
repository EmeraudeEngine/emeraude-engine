/*
 * src/Graphics/Effects/Camera/ToneMapping.cpp
...
/* Local inclusions. */
#include "Graphics/Effects/Camera/VeilingGlare.hpp"
...
namespace
{
	 ...
	 * term the retired VeilingGlare composite pass used to apply.
	 */
	std::string
	withGlareDefine (const char * source) noexcept
	{
...
#ifdef EM_BLOOM
	/* Additive glare, exactly what the retired VeilingGlare composite pass applied
...
namespace EmEn::Graphics::Effects::Camera
{
	...
	ToneMapping::createPipelines () noexcept
	{
		 ...
		 * tonemap shaders (the retired VeilingGlare composite pass, folded here). */
		const bool withGlare = m_glareSource != nullptr;

		/* Compile fragment shaders. */
		auto tmFragment = withGlare
			? shaderManager.getShaderModuleFromSourceCode(device, "ToneMappingGlareFS", ShaderType::FragmentShader, withGlareDefine(ToneMappingFragmentShader))
		...
		auto autoExpFragment = withGlare
			? shaderManager.getShaderModuleFromSourceCode(device, "AutoExpToneMappingGlareFS", ShaderType::FragmentShader, withGlareDefine(AutoExposureToneMappingFragmentShader))
		...
		auto tonemapInputLayout = withGlare ? dualInputLayout : singleInputLayout;
		auto autoExpInputLayout = withGlare ? this->getInputLayout(3) : dualInputLayout;
	...
	ToneMapping::createDescriptorSets () noexcept
	{
		...
		{
			const auto tonemapInputLayout = m_glareSource != nullptr ? dualInputLayout : singleInputLayout;
		...
		{
			const auto autoExpInputLayout = m_glareSource != nullptr ? this->getInputLayout(3) : dualInputLayout;
	...
	ToneMapping::execute (const CommandBuffer & commandBuffer, const TextureInterface & inputColor, const FrameContext & context) noexcept
	{
		...
		if ( m_glareSource != nullptr )
		{
			bloomIntensity = context.camera != nullptr ? context.camera->bloomIntensity() : m_glareSource->parameters().intensity;
		}
		...
		if ( autoExposureEnabled && !m_lumTargets.empty() )
		{
			...
			/* ---- Auto-Exposure Tone Mapping ---- */
			{
				...
				if ( m_glareSource != nullptr )
				{
					static_cast< void >(m_autoExpDescPerFrame[frameIndex]->writeCombinedImageSampler(2, m_glareSource->bloomTexture()));
				}
		...
		else
		{
			...
			if ( m_glareSource != nullptr )
			{
				static_cast< void >(m_tonemapDescPerFrame[frameIndex]->writeCombinedImageSampler(1, m_glareSource->bloomTexture()));
			}
