/*
 * src/Graphics/Effects/Camera/VeilingGlare.cpp
...
static constexpr auto TracerTag{"VeilingGlareEffect"};
...
namespace EmEn::Graphics::Effects::Camera
{
	...
	bool
	VeilingGlare::createPipelines () noexcept
	{
		...
		const auto downsampleFragment = shaderManager.getShaderModuleFromSourceCode(device, "VeilingGlareDownsampleFS", ShaderType::FragmentShader, DownsampleFragmentShader);
		...
		const auto upsampleFragment = shaderManager.getShaderModuleFromSourceCode(device, "VeilingGlareUpsampleFS", ShaderType::FragmentShader, UpsampleFragmentShader);
		...
		const auto compositeFragment = shaderManager.getShaderModuleFromSourceCode(device, "VeilingGlareCompositeFS", ShaderType::FragmentShader, CompositeFragmentShader);
		...
		const StaticVector< VkPushConstantRange, 4 > pushConstantRanges{
			VkPushConstantRange{
				...
				.offset = 0,
				.size = sizeof(VeilingGlarePushConstants)
			}
		};
		...
		m_downsamplePipeline = this->createFullscreenPipeline(ClassId, "VeilingGlareDownsample", vertexModule, downsampleFragment, m_downsampleLayout, m_downTargets[0]);
		m_upsamplePipeline = this->createFullscreenPipeline(ClassId, "VeilingGlareUpsample", vertexModule, upsampleFragment, m_upsampleLayout, m_upTargets[0]);
		m_compositePipeline = this->createFullscreenPipeline(ClassId, "VeilingGlareComposite", vertexModule, compositeFragment, m_compositeLayout, m_outputTarget);
	...
	bool
	VeilingGlare::createDescriptorSets () noexcept
	{
	...
	bool
	VeilingGlare::create (uint32_t width, uint32_t height) noexcept
	{
		...
		for ( uint32_t mipLevel = 0; mipLevel < MipLevels; ++mipLevel )
		{
			...
			if ( !m_downTargets[mipLevel].create(renderer, mipWidth, mipHeight, format, "VeilingGlareDown" + std::to_string(mipLevel)) )
			{
		...
		for ( uint32_t mipLevel = 0; mipLevel < MipLevels - 1; ++mipLevel )
		{
			...
			if ( !m_upTargets[mipLevel].create(renderer, upWidth, upHeight, format, "VeilingGlareUp" + std::to_string(mipLevel)) )
			{
		...
		if ( !m_outputTarget.create(renderer, width, height, format, "VeilingGlareOutput") )
		{
	...
	VeilingGlare::execute (const CommandBuffer & commandBuffer, const TextureInterface & inputColor, const FrameContext & context) noexcept
	{
		...
		for ( uint32_t mipLevel = 0; mipLevel < MipLevels; ++mipLevel )
		{
			...
			const VeilingGlarePushConstants pc{
			...
			IndirectPostProcessEffect::recordFullscreenPass(
				...
				*m_downsampleLayout,
				descSet,
				&pc,
				sizeof(VeilingGlarePushConstants)
			);
		}
