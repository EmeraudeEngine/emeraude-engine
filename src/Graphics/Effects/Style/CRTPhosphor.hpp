/*
 * src/Graphics/Effects/Style/CRTPhosphor.hpp
...
{
	/**
	 * @brief The CRT phosphor grid lens effect class.
	 * @extends EmEn::Graphics::DirectPostProcessEffect This is a framebuffer effect.
	 *
	 * Simulates the visible RGB phosphor grid (aperture grille / shadow mask)
	 * pattern on CRT monitors, where individual red, green and blue phosphor
	 * columns are visible at close viewing distances.
	 */
	class EMEN_API CRTPhosphor final : public DirectPostProcessEffect
	{
		public:

			/** @brief Class identifier. */
			static constexpr auto ClassId{"CRTPhosphor"};

			/**
			 * @brief Constructs a CRT phosphor grid lens effect.
			 * @param intensity The grid visibility (0 = invisible, 1 = full mask). Default 0.25.
			 */
			explicit
			CRTPhosphor (float intensity = 0.25F) noexcept
			...
			/**
			 * @brief Returns the phosphor grid visibility.
			 * @return float
			 */
			[[nodiscard]]
			float
			...
			/**
			 * @brief Returns the phosphor triad scale.
			 * @return float
			 */
			[[nodiscard]]
			float
