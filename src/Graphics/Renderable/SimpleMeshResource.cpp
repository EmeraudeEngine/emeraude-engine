...
namespace EmEn::Graphics::Renderable
{
	...
	SimpleMeshResource::load (Resources::AbstractServiceProvider & serviceProvider, const Json::Value & data) noexcept
	{
		...
		{
			TraceError{ClassId} << "No material definition found ! Expected '" << MeshResource::LayersKey << "' array or '"
				<< MeshResource::MaterialTypeKey << "'/'" << MeshResource::MaterialNameKey << "' keys.";

			return this->setLoadSuccess(false);
		}

		/* Parse material from the layer data. */
		auto materialResource = MeshResource::parseLayer(serviceProvider, *layerData);
