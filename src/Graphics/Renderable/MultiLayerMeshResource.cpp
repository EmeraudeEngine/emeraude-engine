...
namespace EmEn::Graphics::Renderable
{
	...
	bool
	MultiLayerMeshResource::isOpaque (uint32_t layerIndex) const noexcept
	{
	...
	bool
	MultiLayerMeshResource::requiresGrabPass (uint32_t layerIndex) const noexcept
	{
		if ( layerIndex >= static_cast< uint32_t >(m_layers.size()) )
		{
			return false;
	...
	MultiLayerMeshResource::layerRasterizationOptions (uint32_t layerIndex) const noexcept
	{
		if ( layerIndex >= static_cast< uint32_t >(m_layers.size()) )
		{
	...
	bool
	MultiLayerMeshResource::load () noexcept
	{
	...
	bool
	MultiLayerMeshResource::load (const std::filesystem::path & filepath) noexcept
	{
	...
	MultiLayerMeshResource::parseGeometry (const Json::Value & data) noexcept
	{
	...
	MultiLayerMeshResource::parseLayerOptions (const Json::Value & data) noexcept
	{
	...
	bool
	MultiLayerMeshResource::load (const Json::Value & data) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
		...
		for ( const auto & layerRule : layerRules )
		{
			...
			auto materialResource = MultiLayerMeshResource::parseLayer(this->serviceProvider(), layerRule);
			...
			if ( !this->addMaterial(materialResource, MultiLayerMeshResource::parseLayerOptions(layerRule), 0) )
			{
	...
	bool
	MultiLayerMeshResource::load (const std::shared_ptr< Geometry::Interface > & geometry, const std::shared_ptr< Material::Interface > & material, const RasterizationOptions & rasterizationOptions) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
	...
	bool
	MultiLayerMeshResource::load (const std::shared_ptr< Geometry::Interface > & geometry, const std::vector< std::shared_ptr< Material::Interface > > & materialList, const std::vector< RasterizationOptions > & rasterizationOptions) noexcept
	{
		if ( !this->beginLoading() )
		{
			return false;
	...
	bool
	MultiLayerMeshResource::addMaterial (const std::shared_ptr< Material::Interface > & material, const RasterizationOptions & options, uint32_t flags) noexcept
	{
		if ( material == nullptr )
		{
	...
	void
	MultiLayerMeshResource::generateLODLevel (const std::shared_ptr< IndexedVertexResource > & sourceGeometry, uint32_t LODLevel, float ratio) noexcept
	{
