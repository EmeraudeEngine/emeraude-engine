...
namespace EmEn::Graphics::Renderable
{
	...
	class EMEN_API TerrainResource final : public Abstract, public Scenes::GroundLevelInterface
	{
		...
		public:
			...
			TerrainResource (Resources::AbstractServiceProvider & serviceProvider, std::string name, uint32_t resourceFlags = 0) noexcept
				: Abstract{serviceProvider, std::move(name), resourceFlags},
				  m_geometry{std::make_shared< Geometry::CDLODTerrainResource >(serviceProvider, this->name() + "CDLOD")},
				  m_localData{std::make_shared< Base::VertexFactory::Grid< float > >()}
			{
			...
			/**
			 * @brief Returns the knobs of the CDLOD geometry.
			 * @return const Geometry::CDLODTerrainParameters &
			 */
			[[nodiscard]]
			const Geometry::CDLODTerrainParameters &
			CDLODParameters () const noexcept
			{
				return m_parameters;
			}
			...
			load (float gridSize, uint32_t gridDivision, const Base::PixelFactory::Pixmap< pixmapData_t > & displacementMap, float displacementFactor, const std::shared_ptr< Material::Interface > & materialResource, const RasterizationOptions & rasterizationOptions = {}, float UVMultiplier = 1.0F) noexcept requires (std::is_arithmetic_v< pixmapData_t >)
			{
				...
				/* 3. Create the CDLOD geometry from local data. */
		...
		private:
			...
			/**
			 * @brief Hands the full grid to the CDLOD geometry, which builds its height pyramid and quadtree.
			 * @note THE single site every load path ends in. The grid is shared, never copied: it is also
			 * what getLevelAt() answers the physics with.
			 * @return bool
			 */
			[[nodiscard]]
			...
			Geometry::CDLODTerrainParameters m_parameters;
	};
}
