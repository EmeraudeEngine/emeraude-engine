...
namespace EmEn::Graphics
{
	 ...
	 */
	class EMEN_API ViewMatrices2DUBO final : public ViewMatricesInterface
	{
		public:
		...
		private:
			 ...
			 * @return std::ostream &
			 */
			friend EMEN_API std::ostream & operator<< (std::ostream & out, const ViewMatrices2DUBO & obj);
			...
			static constexpr auto NearPlaneOffset{26UL};
			/** @brief Offset of the far plane distance in the buffer. */
			static constexpr auto FarPlaneOffset{27UL};
			...
			struct DataState
			{
				Base::Math::Matrix< 4, float > projection; /**< Projection matrix for 2D view. */
				Base::Math::Matrix< 4, float > view; /**< View matrix. */
				Base::Math::Matrix< 4, float > infinityView; /**< View matrix for infinite distance (skybox). */
	 ...
	 * @return std::string
	 */
	EMEN_API std::string to_string (const ViewMatrices2DUBO & obj) noexcept;
}
