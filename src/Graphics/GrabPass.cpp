...
namespace EmEn::Graphics
{
	...
	GrabPass::create (Renderer & renderer, uint32_t width, uint32_t height, VkFormat colorFormat, VkFormat depthFormat, VkFormat normalsFormat, VkFormat materialPropertiesFormat, VkFormat albedoFormat, VkFormat velocityFormat) noexcept
	{
		...
		/* ⚠️ The COLOR grab pass carries a full mip chain, and only it: a rough transmissive
		 * surface samples a blurred copy of the scene behind it (frosted glass), which is a
		 * textureLod() away once the levels exist. KHR_materials_transmission's reference
		 * implementation does exactly this — getTransmissionSample() reads
		 * log2(width) * applyIorToRoughness(roughness, ior) as its LOD.
		 *
		 * ⚠️ Three things must agree or the blur is silently absent: the image needs
		 * TRANSFER_SRC too (the chain is generated by blitting the image onto ITSELF, level N-1
		 * to level N), the view must expose every level, and the sampler's maxLod must reach
		 * them — it used to be pinned at 1.0F, which would have clamped the blur to the first
		 * level even with a chain present.
		 *
		...
		/* Create the color grab pass image with transfer source/destination and sampled usage. */
		...
		/* ⚠️ getSampler() keys on the NAME, so the LOD range must be part of it: a "GrabPass"
		 * sampler cached from a previous swap-chain size would otherwise be handed back with the
		 * wrong maxLod and clamp the blur without a word. */
		m_sampler = renderer.getSampler("GrabPass-" + std::to_string(colorMipLevels), [colorMipLevels] (Settings &, VkSamplerCreateInfo & createInfo) {
			...
			createInfo.maxLod = static_cast< float >(colorMipLevels);
		});
	...
	GrabPass::recordBlit (const CommandBuffer & commandBuffer, const Image & srcColorImage, const Image * srcDepthImage, const Image * srcNormalsImage, const Image * srcMaterialPropertiesImage) const noexcept
	{
		...
		/* === Mip chain: blit the colour grab onto itself, level N-1 -> level N. ===
		 *
		 * ⚠️ This is what makes a ROUGH transmissive surface look frosted — the shader reads a
		 * LOD from the material roughness. Cost: ~1/3 of the base image in extra writes, once per
		 * frame, and only on frames that already pay for the grab pass (it is armed by
		 * Scene::hasTranslucentGBObjects()). ⚠️ Every level is generated, even when every
		 * transmissive material in the frame is perfectly smooth and only reads level 0 — closing
		 * that would need the renderer to know the roughest grab-pass material of the frame. */
		if ( m_colorMipLevels > 1 )
		{
			for ( uint32_t mipLevel = 1; mipLevel < m_colorMipLevels; mipLevel++ )
			{
				/* The level just written becomes the source of the next one. */
				{
					...
					barrier.targetMipLevel(mipLevel - 1);
					barrier.setIdentifier(ClassId, "ColorMipLevelBeforeBlit", "ImageMemoryBarrier");

					commandBuffer.pipelineBarrier(barrier, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
				}

				/* ⚠️ A mip extent never goes below 1: a 2880x1620 image reaches 1 in height five
				 * levels before it does in width, and a zero offset is an invalid blit. */
				...
				imageBlit.srcOffsets[1] = {
					.x = static_cast< int32_t >(std::max(m_image->width() >> (mipLevel - 1), 1U)),
					.y = static_cast< int32_t >(std::max(m_image->height() >> (mipLevel - 1), 1U)),
					.z = 1
				};
				...
				imageBlit.dstOffsets[1] = {
					.x = static_cast< int32_t >(std::max(m_image->width() >> mipLevel, 1U)),
					.y = static_cast< int32_t >(std::max(m_image->height() >> mipLevel, 1U)),
					.z = 1
				};
		...
		/* ⚠️ After the chain the colour grab sits in TWO layouts: levels [0, N-1) were turned into
		 * blit sources, only the LAST one is still a transfer destination. One whole-image barrier
		 * would declare the wrong old layout for most of them. */
		if ( m_colorMipLevels > 1 )
		{
			{
				Sync::ImageMemoryBarrier barrier{
				...
				barrier.targetMipLevel(0, m_colorMipLevels - 1);

				barriers.push_back(barrier.get());
			}

			{
				...
				barrier.targetMipLevel(m_colorMipLevels - 1);
