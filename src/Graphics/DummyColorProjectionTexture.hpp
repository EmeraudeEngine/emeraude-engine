/*
 * src/Graphics/DummyColorProjectionTexture.hpp
...
{
	/**
	 * @brief A dummy color projection texture (1x1, white RGBA = no color filtering).
	 * @note This texture is used when a light doesn't have a color projection texture assigned,
	 * allowing unified descriptor set layouts across all lights. Sampling white is a no-op
	...
	class DummyColorProjectionTexture final : public Vulkan::TextureInterface
	{
		public:
			...
			static constexpr auto ClassId{"DummyColorProjectionTexture"};

			/**
			 * @brief Constructs a dummy color projection texture.
			 ...
			 */
			~DummyColorProjectionTexture () override = default;

			/**
			 * @brief Creates the dummy color projection texture on the GPU.
