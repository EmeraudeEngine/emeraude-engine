...
namespace EmEn::Graphics
{
	...
	Renderer::onInitialize () noexcept
	{
		...
		{
			if ( !m_postProcessor.configure(m_swapChain) )
			{
	...
	Renderer::recreateSceneTarget () noexcept
	{
		...
		const auto colorFormat = m_postProcessor.isHDREnabled()
		...
		const auto normalsFormat = VK_FORMAT_R16G16B16A16_SFLOAT;
		...
		TraceSuccess{ClassId} << "Scene render target created (" << width << "x" << height << ", format: " << (m_postProcessor.isHDREnabled() ? "R16G16B16A16_SFLOAT" : "swapchain") << ").";
	...
	Renderer::renderFrame (const std::shared_ptr< Scenes::Scene > & scene, const Overlay::Manager & overlayManager) noexcept
	{
		...
		if ( m_postProcessor.isEnabled() && m_sceneTarget == nullptr )
		{
			...
			{
				if ( !m_postProcessor.configure(std::static_pointer_cast< RenderTarget::Abstract >(m_sceneTarget)) )
				{
	...
	Renderer::renderFrameDirect (const std::shared_ptr< Scenes::Scene > & scene, const Overlay::Manager & overlayManager, RendererFrameScope & /*currentFrameScope*/, const std::shared_ptr< CommandBuffer > & commandBuffer) noexcept
	{
		...
		if ( m_postProcessor.isEnabled() )
		{
			...
			m_postProcessor.executeEffectChain(*commandBuffer);

			commandBuffer->beginRenderPass(*m_swapChain->postProcessFramebuffer(), m_swapChain->renderArea(), m_swapChainClearColors, VK_SUBPASS_CONTENTS_INLINE);

			m_postProcessor.render(*commandBuffer);
		}
	...
	Renderer::recreateRenderingSubSystem (bool withSurface, bool useNativeCode) noexcept
	{
		...
		{
			if ( !m_postProcessor.configure(m_sceneTarget != nullptr ? std::static_pointer_cast< RenderTarget::Abstract >(m_sceneTarget) : std::static_pointer_cast< RenderTarget::Abstract >(m_swapChain)) )
			{
