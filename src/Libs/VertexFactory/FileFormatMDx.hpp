	...
	class FileFormatMDx final : public FileFormatInterface< vertex_data_t, index_data_t >
	{
		public:
			...
			readFile (const std::filesystem::path & filepath, Shape< vertex_data_t, index_data_t > & geometry, const ReadOptions & readOptions) noexcept override
			{
				...
				{
					return loadMDL(file, geometry, filepath);
				}
				// MD2: "IDP2"
				else if ( ident == (('2' << 24) + ('P' << 16) + ('D' << 8) + 'I') )
				{
					return loadMD2(file, geometry, filepath);
				}
				// MD3: "IDP3"
				else if ( ident == (('3' << 24) + ('P' << 16) + ('D' << 8) + 'I') )
				{
					return loadMD3(file, geometry, filepath);
				}
				...
				{
					return loadMD5(file, geometry, filepath);
				}

				std::cerr << __PRETTY_FUNCTION__ << ", unknown format for file '" << filepath << "'!" "\n";
				return false;
			}
		...
		private:
			...
			struct mdl_skin_t { int group = 0; std::vector< unsigned char > data{}; };
			struct mdl_texCoord_t { int onseam = 0; int s = 0; int t = 0; };
			struct mdl_triangle_t { int facesfront = 0; std::array< int, 3 > vertex{0, 0, 0}; };
			struct mdl_vertex_t { std::array< unsigned char, 3 > v{0, 0, 0}; unsigned char normalIndex = 0; };
			...
			bool loadMDL(std::ifstream& file, Shape< vertex_data_t, index_data_t >& geometry, const std::filesystem::path& filepath)
			{
				mdl_header_t header;
				...
				const auto triangleCount = static_cast< vertex_data_t >(header.num_tris);
			...
			struct md2_vertex_t { std::array< unsigned char, 3 > v{0, 0, 0}; unsigned char normalIndex = 0; };
			struct md2_triangle_t { std::array< unsigned short, 3 > vertex{0, 0, 0}; std::array< unsigned short, 3 > st{0, 0, 0}; };
			struct md2_texCoord_t { short s = 0; short t = 0; };
			struct md2_frame_t { md2_vec3_t scale; md2_vec3_t translate; std::array< char, 16 > name; std::vector< md2_vertex_t > verts; };

			bool loadMD2(std::ifstream& file, Shape< vertex_data_t, index_data_t >& geometry, const std::filesystem::path& filepath)
			{
				md2_header_t header;
				...
				for ( size_t triangleIndex = 0; triangleIndex < triangleCount; triangleIndex++ )
				{
					for ( size_t vertexIndex = 0; vertexIndex < 3; vertexIndex++ )
					{
						...
						builder.setTextureCoordinates(
							...
							skinHeight - (static_cast< vertex_data_t >(textureCoordinate.t) / skinHeight)
						);
						builder.newVertex();
					}
				}
			...
			struct md3_header_t
			{
				int ident; int version; char name[64]; int flags;
				int num_frames; int num_tags; int num_surfaces; int num_skins;
				int offset_frames; int offset_tags; int offset_surfaces; int offset_eof;
			};

			struct md3_surface_t
			{
				int ident; char name[64]; int flags;
				int num_frames; int num_shaders; int num_verts; int num_triangles;
				int offset_triangles; int offset_shaders; int offset_st; int offset_xyzn; int offset_end;
			};

			struct md3_triangle_t { int indexes[3]; };
			struct md3_shader_t { char name[64]; int shader_index; };
			struct md3_texCoord_t { float s; float t; };
			struct md3_vertex_t { short v[3]; unsigned char normal[2]; };

			bool loadMD3(std::ifstream& file, Shape< vertex_data_t, index_data_t >& geometry, const std::filesystem::path& filepath)
			{
				md3_header_t header;
				...
				for(int i=0; i<header.num_surfaces; ++i) {
					...
					file.read(reinterpret_cast<char*>(&surfaces[i]), sizeof(md3_surface_t));
					totalTriangles += surfaces[i].num_triangles;
					currentSurfaceOffset += surfaces[i].offset_end;
				}
					...
					for ( const auto & tri : tris )
					{
						for ( int k = 0; k < 3; ++k ) // Vertices
						{
							...
							builder.setPosition(
								...
								static_cast< vertex_data_t >(v.v[2]) * MD3_XYZ_SCALE, // Swap Y/Z for OpenGL? Usually needed.
								static_cast< vertex_data_t >(v.v[1]) * MD3_XYZ_SCALE
							);
							builder.setNormal(
								static_cast< vertex_data_t >(nx),
								static_cast< vertex_data_t >(nz), // Swap Y/Z
								static_cast< vertex_data_t >(ny)
							);
							builder.setTextureCoordinates(
								static_cast< vertex_data_t >(uv.s),
								static_cast< vertex_data_t >(1.0f - uv.t)
							);
							builder.newVertex();
						}
					}
				...
				geometry.transform(Math::Matrix< 4, vertex_data_t >::rotation(Math::Radian< vertex_data_t >(-90), 1, 0, 0)); // Fix orientation
				builder.endConstruction();
				return true;
			}
			...
			// Quaternion helpers
			void computeW(std::array<float, 4>& q)
			{
				float t = 1.0f - (q[0] * q[0] + q[1] * q[1] + q[2] * q[2]);
				q[3] = (t < 0.0f) ? 0.0f : -std::sqrt(t);
			}

			void rotatePoint(const std::array<float, 4>& q, const std::array<float, 3>& in, std::array<float, 3>& out)
			{
			...
			bool loadMD5(std::ifstream& file, Shape< vertex_data_t, index_data_t >& geometry, const std::filesystem::path& filepath)
			{
				std::string line;
				...
				{
					if (line.find("numJoints") != std::string::npos) std::stringstream(line) >> line >> numJoints;
					else if (line.find("numMeshes") != std::string::npos) std::stringstream(line) >> line >> numMeshes;
				...
				for(const auto& mesh : meshes)
				{
					...
					{
						for(int k=0; k<3; ++k) // For each vertices in triangle
						{
							// Order: 0, 2, 1 usually for MD5 winding? Or 0, 1, 2?
							// Doom3 uses standard winding, but often we need to check. Keeping 0,1,2.
							int vIdx = tri[k];
							...
							builder.setPosition(
								...
								static_cast< vertex_data_t >(finalPos[2]), // Swap Y/Z
								static_cast< vertex_data_t >(finalPos[1])
							);
							// No normals in MD5 mesh, usually computed from smoothing groups or provided in animation? 
							// Actually MD5 only has weights. Normals must be computed or are in weights?
							// Standard MD5 mesh does not store vertex normals, they are computed from weights/joints or recalculate.
							// For this static loader, we will simply provide a default or Up normal, 
							// or better, let the ShapeBuilder compute normals if we don't provide them, 
							// OR calculate flat normals per triangle.
							// Currently just setting 0,1,0 as placeholder.
				...
				geometry.transform(Math::Matrix< 4, vertex_data_t >::rotation(Math::Radian< vertex_data_t >(-90), 1, 0, 0));
				builder.endConstruction();
				return true;
			}
