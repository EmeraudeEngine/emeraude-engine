...
namespace EmEn::Libs::WaveFactory
{
	...
	class Synthesizer final
	{
		public:
			...
			/**
			 * @brief Generates a constexpr sine lookup table.
			 * @return std::array< float, SinTableSize > The sine table covering [0, 2π).
			 */
			static constexpr std::array< float, SinTableSize >
			...
			/** @brief Pre-computed sine lookup table. */
			static constexpr std::array< float, SinTableSize > SinTable = generateSinTable();
