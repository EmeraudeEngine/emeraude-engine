...
| `EMERAUDE_USE_EXPLICIT_EXPORTS` | `Off` | Windows: explicit `EMEN_API` export macro instead of `WINDOWS_EXPORT_ALL_SYMBOLS`. Migration complete and verified; `Off` by default because `On` makes **the consuming application's link much longer** (on every link) and requires annotating every new consumer-referenced public symbol. `On` is the only way to get a PCH on the engine target under MSVC — a bad trade against that link cost. See [`docs/windows-export-api.md`](docs/windows-export-api.md). |
