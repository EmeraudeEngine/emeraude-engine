...
- SHADERS CODE GENERATION: **shader caches — audited Aug 2026, two items left.**
  The three stages are settled and measured; only the naming and the housekeeping are open.
  ...
  § "VkPipelineCache".
  REMAINING:
  1. Rename `EnableSourceCodeCache` to say what it is: a debug DUMP, not a cache. Nothing ever
     reads it back (`AbstractShader::loadSourceCode()` has zero callers) and its key is a hash of
     the source it stores, so it structurally cannot be one. It writes one sub-directory per
     generator (SceneRendering/, ShadowCasting/, PostProcessing/, OverlayRendering/,
     GizmoRendering/, TBNSpaceRendering/); its only purpose is inspecting what the generators
     produced, and it stays OFF by default.
  2. Add pruning: nothing cleans the cache directories except the `--clear-shader-cache` option
     (which clears the shader caches including the pipeline cache).
