...
- SHADERS CODE GENERATION: **shader caches — audited Aug 2026, one item left.**
  ...
  false.
  REMAINING:
  1. Add pruning: nothing cleans the cache directories except the `--clear-shader-cache` option
     (which clears the shader caches including the pipeline cache). It must also cover the stale
     `~/.cache/<app>/shader-sources/` directory left behind on every machine that had the dump
     enabled before the rename — nothing will ever clean it otherwise.
