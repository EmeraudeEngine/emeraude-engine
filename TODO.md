...
- LIGHTING AND SHADOWING: **Normalise the legacy SPECULAR term in ENERGY** (Jul 2026). The legacy
  (non-PBR) specular is emitted as `specularColor * illuminance * pow(max(dot(R, V), 0), shininess)`
  with **no normalisation factor at all**, while its diffuse sibling now carries the Lambertian
  `1/pi` (see `docs/caution-points.md`, "Direct diffuse was missing the Lambertian 1/pi"). The
  highlight energy therefore grows without bound as `shininess` rises instead of staying constant,
  and a legacy material cannot be compared to a PBR one lit by the same source.
  - The model is **Phong**, not Blinn-Phong: the factor is built from `reflect(rayDirection, N)`
    dotted with the view vector (`LightGenerator.PerFragment.cpp:414`,
    `LightGenerator.PerFragment.NormalMap.cpp:412`, `LightGenerator.PerVertex.cpp:146`), so the
    normalisation is **`(n + 2) / (2 * pi)`** — NOT the `(n + 2) / (8 * pi)` that belongs to a
    half-vector (Blinn) formulation. Getting this wrong is a factor of 4.
  - Emission sites to change, all in `LightGenerator::generateFinalFragmentOutput()`:
    `LightGenerator.cpp:1318` (reflection+refraction), `:1324` (reflection), `:1328` (refraction),
    `:1332` (plain). Mirror what was done for the diffuse: build the scaled expression once next to
    `finaleSpecularFactor`, do NOT fold it into `finaleSpecularFactor` itself.
  - ⚠️ With `shininess = 192` (e.g. `Grounds/desert001`) the factor is ~30.9, so normalising makes
    tight highlights **much brighter**, not dimmer. Expect to re-tune `Shininess` and `Specular`
    across the material store, and to re-check every demo's exposure afterwards.
  - ⚠️ Also review the PBR low-quality specular approximation right below (`LightGenerator.cpp:~1340`),
    which reuses the raw N.L `finaleDiffuseFactor` inside its own `pow()` and is likewise unnormalised.
  - **Owner decision required before implementing:** this changes the look of every legacy material
    in every demo. Not a silent edit.
