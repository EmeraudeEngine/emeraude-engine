...
- SHADERS CODE GENERATION: **shader caches — audited Aug 2026, two items left.**
  Done: the "source code cache" is a DUMP (nothing reads it back, and its key is a hash of the
  source it stores so it structurally cannot be one) — it now writes one sub-directory per
  generator and runs before the binary-cache check; a `std::stoull` crash on stray cache files was
  fixed (the cascade builds `-fno-exceptions`, so a file named `foo_bar.vert` dropped in the
